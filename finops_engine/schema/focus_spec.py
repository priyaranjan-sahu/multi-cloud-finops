...
class FocusRecord(BaseModel):
    ...
    sub_account_id: str = Field(default="default-account", description="AWS Account ID, GCP Project ID, Azure Subscription ID")
    resource_id: Optional[str] = Field(default=None, description="ARN, instance ID, or resource URI")
...
def normalize_to_focus_dataframe(records: List[FocusRecord]) -> pd.DataFrame:
    ...
    if not records:
        return pd.DataFrame(columns=[
            "provider_name", "publisher_name", "charge_category", "billed_cost",
            "effective_cost", "currency", "usage_quantity", "usage_unit",
            "service_name", "service_category", "region_id", "sub_account_id",
            "resource_id", "billing_period_start", "billing_period_end"
        ])
