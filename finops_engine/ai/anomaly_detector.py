...
class AnomalyDetector:
    ...
    def detect_anomalies(self, records: List[FocusRecord]) -> List[Dict[str, Any]]:
        ...
        daily["mean"] = daily.groupby(["provider_name", "service_name"])["billed_cost"].transform(lambda x: x.rolling(7, min_periods=1).mean())
        daily["std"] = daily.groupby(["provider_name", "service_name"])["billed_cost"].transform(lambda x: x.rolling(7, min_periods=1).std().fillna(1.0))
        ...
        for _, row in anomalies_df.iterrows():
            ...
            results.append({
                ...
                "root_cause": f"Unusual spend spike detected in {row['provider_name']} service {row['service_name']} (${excess_cost} above baseline)"
            })
